...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    LOG_DBG("mouse move press position %d keycode 0x%02X", event.position, binding->param1);
...
static int on_keymap_binding_released(struct zmk_behavior_binding *binding,
                                      struct zmk_behavior_binding_event event) {
    LOG_DBG("mouse move released position %d keycode 0x%02X", event.position, binding->param1);
    LOG_DBG("mouse move x %d y %d", MOVE_HOR_DECODE(binding->param1),
            MOVE_VERT_DECODE(binding->param1));
    const struct device *dev = device_get_binding(binding->behavior_dev);
    const struct mouse_config *config = dev->config;
